const Dashboard = {
  ...
  async loadEvents() {
    try {
      const response = await fetch("../api/events/dashboard-events.php");

      const raw = await response.text();

      console.log("DASHBOARD EVENT RAW:", raw);
      ...
      try {
        ...
        console.error("Dashboard API bukan JSON:", raw);
      ...
      if (!data.success) {
        throw new Error(
          data.message || data.error || "Gagal mengambil data event.",
        );
      }
      ...
      this.renderLatestEvents(data.latestEvents || []);
    } catch (error) {
      console.error("DASHBOARD EVENT ERROR:", error);
  ...
  async loadFeaturedEvent() {
    try {
      const response = await fetch(this.FEATURED_EVENT_API);

      const raw = await response.text();

      console.log("FEATURED EVENT RAW:", raw);
      ...
      try {
        ...
        console.error("Featured event API bukan JSON:", raw);
      ...
      if (data.status !== "success" || !data.event) {
        this.renderFeaturedEvent(null);

        return;
      }

      this.renderFeaturedEvent(data.event, data.type);
    } catch (error) {
  ...
  async loadPressReleases() {
    ...
    const recentTable = document.getElementById("recentArticles");

    try {
      const response = await fetch(this.PRESS_RELEASE_API);

      const raw = await response.text();

      console.log("PRESS RELEASE RAW:", raw);
      ...
      try {
        ...
        console.error("Press release API bukan JSON:", raw);
      ...
      if (!data.success) {
        throw new Error(data.message || "Gagal mengambil data press release.");
      }
      ...
      if (totalArticles) {
        totalArticles.textContent = data.data?.total ?? 0;
      }
      ...
      this.renderRecentArticles(items.slice(0, 5));
    } catch (error) {
  ...
  renderLatestEvents(events) {
    ...
    container.innerHTML = events
      ...
      .map((item) => {
        ...
        if (item.category === "Others" && item.category_name) {
        ...
        const categoryClass = slugifyCategory(item.category || "");
                        ...
                        ${
                          image
                            ...
                            <img
                                ...
                                alt="${escapeHtml(item.title || "Event")}"
                            >
                            `
                            : `
                            <div
  ...
  renderFeaturedEvent(event, type = "featured") {
    ...
    const image = event.cover_image ? ROOT_PATH + "/" + event.cover_image : "";

    const description =
      typeof truncateWords === "function"
        ? truncateWords(event.description || "", 16)
        : event.description || "";
    ...
    const stateLabel = type === "upcoming" ? "Upcoming (auto)" : "Featured";
                ...
                ${
                  image
                    ...
                    <img
                        ...
                        alt="${escapeHtml(event.title || "Featured Event")}"
                    >
                    `
                    : ""
                }
  ...
  renderRecentArticles(items) {
    ...
    container.innerHTML = items
      .map((item) => {
        ...
        const imageUrl = normalizePressReleaseImage(item.cover_image);
        ...
        const date = formatPressReleaseDate(
